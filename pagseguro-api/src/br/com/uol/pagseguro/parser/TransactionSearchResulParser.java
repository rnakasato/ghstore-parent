			...
			if( "date".equals( currentElement ) ) {
				...
				try {
					transactionSearchResult
							.setDate( PagSeguroUtil.parse( buf.append( buffer, start, length ).toString() ) );
				...
				transactionSearchResult
						.setResultsInThisPage( Integer.parseInt( buf.append( buffer, start, length ).toString() ) );
			} else if( "totalPages".equals( currentElement ) ) {
				transactionSearchResult
						.setTotalPages( Integer.parseInt( buf.append( buffer, start, length ).toString() ) );
			}
		...
		if( "transaction".equals( qName ) ) {
			...
			try {
				...
				Transaction transaction = TransactionParser
						.readTransaction( new ByteArrayInputStream( xmlTransaction.toString().getBytes() ) );
