...
public class PaymentMB {
	public String getShippingCost( ShoppingCart shoppingCart ) {
		...
		try {
			if( ListUtils.isEmpty( shoppingCart.getShoppingCartList() ) || shoppingCart.getAddress() == null ) {
				shippingCost = "0.00";
			} else {
			...
			shippingCost = "0.00";
			// Para propósitos de teste caso o WS não esteja disponível o Frete
			// é igual a 0
		}
	...
	public String getTotalPayment( ShoppingCart shoppingCart ) {
		...
		try {
			...
			if( ListUtils.isEmpty( shoppingCart.getShoppingCartList() ) ) {
				totalPayment = "0.00";
			} else if( shoppingCart.getAddress() == null ) {
				if( shoppingCart.getTotalValue() != null ) {
					totalPayment = shoppingCart.getTotalValue().toString();
				} else {
					totalPayment = "0.00";
				}
