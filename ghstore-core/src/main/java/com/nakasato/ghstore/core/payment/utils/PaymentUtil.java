...
public class PaymentUtil {
	...
	public static String createPayment(ShoppingCart shoppingCart, User loggedUser){
		...
		for(ShoppingCartItem item : shoppingCart.getShoppingCartList()){
			...
			paymentItem.setWeight(shoppingCart.getTotalWeight());// Peso em gramas
			checkout.addItem(paymentItem);
		}
		checkout.setShippingCost(new BigDecimal(FormatUtils.formatToCurrencyNoSymbol(FreteUtil.getShippingCost(shoppingCart))));
